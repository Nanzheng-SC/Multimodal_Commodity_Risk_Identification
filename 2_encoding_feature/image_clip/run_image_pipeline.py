...
from image_clip.config import image_output_config
from image_clip.dataset import ImageDataset
from image_clip.encoder_local import ImageEncoder
from image_clip.pooling import ImagePooling
...
def _load_structured_index(frequency: str):
    ...
    print(f"Warning: structured index file not found for frequency={frequency}. Using image-only index.")
    return None


def _derive_monthly_from_daily_if_available() -> pd.DataFrame | None:
    daily_dir = features_dir("image", "daily")
    daily_embeddings = os.path.join(daily_dir, "image_embeddings.npy")
    daily_index = os.path.join(daily_dir, "image_index.npy")
    daily_counts = os.path.join(daily_dir, "image_counts.npy")
    daily_flags = os.path.join(daily_dir, "image_missing_flags.npy")
    ...
    frame = pd.DataFrame(
        {
            ...
            "image_count": counts,
            "image_missing_flag": flags,
        }
    )
    frame["month"] = pd.to_datetime(frame["date"], errors="coerce").dt.strftime("%Y-%m")
    monthly_rows = []
    for month, group in frame.groupby("month"):
        valid = group[group["image_count"] > 0]
        if valid.empty:
            monthly_rows.append({"month": month, "embedding": np.zeros_like(frame.iloc[0]["embedding"]), "image_count": 0, "image_missing_flag": 1})
            continue
        weight = valid["image_count"].to_numpy(dtype=np.float32)
        stacked = np.stack(valid["embedding"].to_list()).astype(np.float32)
        monthly_rows.append(
            {
                "month": month,
                "embedding": np.average(stacked, axis=0, weights=weight),
                "image_count": int(valid["image_count"].sum()),
...
def run_image_pipeline(frequency: str = "monthly"):
    frequency = validate_frequency(frequency)
    index_column = index_column_name(frequency)
    structured_index = _load_structured_index(frequency)
    output_config = image_output_config(frequency)
    output_dir = output_config["features_dir"]
    os.makedirs(output_dir, exist_ok=True)

    if frequency == "monthly":
        derived = _derive_monthly_from_daily_if_available()
    else:
        derived = None

    if derived is not None:
        results_df = derived.copy()
        input_samples = int(results_df["image_count"].sum())
        valid_samples = int((results_df["image_count"] > 0).sum())
        missing_samples = int(len(results_df) - valid_samples)
    else:
        dataset = ImageDataset(frequency=frequency)
    ...
    if structured_index is not None:
        ...
        results_df["image_count"] = results_df["image_count"].fillna(0).astype(int)
        results_df["image_missing_flag"] = results_df["image_missing_flag"].fillna(1).astype(int)
    ...
    counts = results_df["image_count"].values
    missing_flags = results_df["image_missing_flag"].values
    ...
    legacy_index_file = os.path.join(output_dir, "image_months.npy")
    ...
    print("=== Image encoding and aggregation summary ===")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the image encoding pipeline.")
...
if __name__ == "__main__":
    args = parse_args()
    run_image_pipeline(frequency=args.frequency)
